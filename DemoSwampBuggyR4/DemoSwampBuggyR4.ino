/////////////////////////////////////////////////////////////////
// Water Game Demo Swamp Buggy Example Sketch
//
// This sketch is provided as a sample to help get your started
// See our Demo Swamp Buggy profile on the website for more
//
/////////////////////////////////////////////////////////////////
...
const char *name = "19743Swamp"; // Change this to something unique to your team
...
const int bucketDown = 120;
const int bucketUp = 200;
const int armDown = 132;
const int armUp = 36;
    ...
    while (central.connected()) { // while the central is still connected to peripheral
      ...
      if (msg.id == "bup") { // button is called "bup" for Button Up
        if (msg.value.toInt() == 1) { // if they clicked it
          ...
          analogWrite(arm, armUp);
        }
      }
      if (msg.id == "bdown") { 
        if (msg.value.toInt() == 1) { // move servos to down (collect) positions
          analogWrite(bucket, bucketDown);
          analogWrite(arm, armDown);
        }
      }
      if (msg.id == "bdrop") { 
        if (msg.value.toInt() == 1) { // arm servo should already be up, so release the bucket to drop the duck
          analogWrite(bucket, bucketDown);
        }
      } 
    /************************************************************/
